...
4. **本月日历**：中英文评分分别显示金币/银币/铜币（≥90 金币、≥80 银币、其余铜币）；绿色=两项有评分，黄色=一项有评分，灰=无评分，今天有橙色描边。
   日历标题旁同步显示当前月份累计获得的金、银、铜币数量。
...
### 系统特性
- 当前录音保留 **90 天**自动删除（服务启动 + 每 6 小时扫描一次，删除后记录字段清空，后台显示「音频已过期」）。
- 覆盖上传前，旧录音自动归档到 `uploads/_backup/{date}/`，不删除；备份不参与 90 天自动清理（自定义 `UPLOAD_DIR` 时位于该目录下的 `_backup/{date}/`）。
...
- 录音总时长不足 3 分钟（`< 180` 秒）不打分，也不计入日历奖牌和本月币统计。
- ffmpeg 跳过开头静音并截取首个 10 秒语音，转成 16kHz/16bit/单声道 WAV；有效语音不足 10 秒时显示「暂未评分」。
- 使用新版 WSS 接口自由说模式（`eval_mode=3`、无参考文本），中文/英文分别使用 `16k_zh` / `16k_en`；中文评测苛刻指数 `score_coeff=2.5`（中间档），英文为 `score_coeff=1.0`（儿童档）；总分 = (发音准确度 + 流利度×100) / 2，失败最多重试两次。
